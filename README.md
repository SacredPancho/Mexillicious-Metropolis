...
- `prototypes/metropolis-3d.html` — **the primary renderer.** WebGL via three.js r128
  ...
  a district. Thin bevelled plinths on faceted terrain, Kenney CC0 models tinted pastel,
  raked soft shadows, SSAO, depth of field, bloom, two moons. Night only. `?d=16` starts
  closer in; `?ao=0` and `?dof=0` switch passes off.
- `prototypes/models.js` — the models as data URIs, generated by `tools/pack-models.py`
  from `prototypes/models/`. Regenerate after adding a model.
...
**Modelled pieces come from CC0 kits, tinted into the palette.** Kenney's city and
platformer kits (`prototypes/models/kenney/`, CC0) supply buildings, tree tiles, a fountain
and the cadet character. They ship in saturated orange, green and purple, so a shader hook
mixes every model a third of the way to white and adds a little self-glow; that is what
lets them sit beside the sugar plinths. The renderer never depends on a model being
present: every placement has a procedural fallback.

**The ground is terrain, not water, and plinths are slabs, not blocks.** A mirror floor
read as a lake around an island. The ground is now flat under the city and faceted
